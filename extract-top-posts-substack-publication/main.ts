...
flow.create('a1b2c3', 'Extract Top Posts Substack Publication', function (f) {
  f.node('c01000', 'Core.Flow.Comment', 'Comment', { optText: '### Substack Top Posts\n\nExtracts a Substack\'s top-performing posts — title, description, date, author, likes, comments, and URLs — into a CSV.\n\nEnter a publication URL when prompted.' });
  ...
  f.node('111111', 'Core.Trigger.Inject', 'Start', {})
    .then('222222', 'Core.Programming.Function', 'Setup Vars', {
      func: `
        ...
        msg.archive_url = base + '/archive?sort=top';
        msg.csv_path = global.get('$Home$') + '/extract-top-posts-substack-publication.csv';
        return msg;
      `
    })
    ...
    .then('444444', 'Core.Browser.OpenLink', 'Open Archive Top', {
    ...
    .then('666666', 'Core.Browser.RunScript', 'Extract Posts', {
      ...
      func: `
        ...
        for (var i = 0; i < titleAnchors.length; i++) {
          ...
          var authorA = c.querySelector('a.link-HFGLqU');
          if (!authorA) authorA = c.querySelector('a[href*="/@"]');
          ...
          rows.push({
            ...
            'Post URL': titleA.href,
            'Author Profile URL': authorA ? authorA.href : '',
            'Post Image URL': img ? img.src : ''
          });
        }
        var columns = ['Position','Title','Description','Date','Author','Likes','Comments Count','Post URL','Author Profile URL','Post Image URL'];
