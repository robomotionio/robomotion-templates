...
flow.create('681af335-947c-40e8-d2b6-cf537ea94081', 'AI Overview Keyword Wins by Email', (f) => {
  ...
  f.node('a1f307', 'Core.Flow.Comment', 'About', {
    optText: '### Get new ranked Google AI Overview keywords via email\n' +
      'Each week, find the keywords where your domains are newly cited inside Google\'s AI Overview, save the full picture to Google Sheets, and email the new placements with position, search volume and ranking URL.\n' +
      'An AI Overview citation is becoming the position-zero of the AI era. Tracking it weekly tells you which of your pages Google trusts as a source.'
  });
...
// Only keywords whose SERP shows an AI Overview that cites this domain.
msg.filters = [['ranked_serp_element.serp_item.type', '=', 'ai_overview']];
msg.alert_title = 'New AI Overview placements';
...
return msg;`
  ...
  f.node('e91daf', 'Core.Flow.Comment', 'Pull current rankings', {
    optText: '#### Pull current rankings\n' +
      'One pass per target domain, filtered to AI Overview placements, paging 1000 at a time.'
  });
...
return [null, msg];`
  ...
  f.node('cf1826', 'Core.Flow.Comment', 'Diff and announce', {
    optText: '#### Diff and announce\n' +
      'Every current AI Overview placement is written back to the sheet. Only the new ones are emailed.'
  });
...
msg.mail_subject = msg.alert_title + ' - ' + msg.target_row.target + ' - ' + msg.today;
msg.announcement = '<html><body><p>' + msg.alert_title + ' for ' +
  msg.target_row.target + ':</p><ul>';
for (var n = 0; n < msg.new_rows.length; n++) {
  var r = msg.new_rows[n];
  msg.announcement += '<li><b>' + r.Keyword + '</b> - position ' + r.Rank +
    ', ' + r['Search Volume'] + ' searches/mo - <a href="' + r.URL + '">' + r.URL + '</a></li>';
}
msg.announcement += '</ul></body></html>';
return [msg, null];`
  });
  ...
  f.node('7c04e9', 'Robomotion.Gmail.Messages.Send', 'Email The Wins', {
    inTo: Message('notify_email'),
    inSubject: Message('mail_subject'),
    inBody: Message('announcement'),
    optIsHTML: true,
    optCredentials: { vaultId: '_', itemId: '_' }
  });
