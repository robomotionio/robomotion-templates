...
Several nodes have multiple output ports. See `loops.md` for port reference.
...
f.node('a07c4e', 'Core.Net.HttpRequest', 'Fetch Data', {
  inUrl: Custom('https://api.example.com/data'),
...
- **New flows:** Get the version from `plan_flow` response or `generate_dependencies`.
