# GoHighLevel Lead Business Data

Every 15 minutes, takes the leads GoHighLevel has touched most recently, turns each website into a bare domain, looks the company up in the Google Business listings database, and writes the name, description, categories, address, rating and price level back onto the GoHighLevel record.
...
1. **Every 15 Minutes** — an Inject trigger on the source scenario's 900-second interval. **Set Parameters** holds
   ...
   enrichment value lands on the GoHighLevel side.
...
3. **List Recent Contacts** — reads the ten most recently touched records via the `Contacts.Search` node, sorted by `date_updated`.
...
6. **Update Contact** — writes back through an HTTP PUT to `/contacts/{id}` with a `customFields` array — the package Update node does not expose custom fields.
...
- **GoHighLevel credential** — a vault item holding the API token.
...
- [Get Business Data for GoHighLevel leads with DataForSEO + Make](https://dataforseo.com/templates/get-business-data-for-gohighlevel-leads-with-dataforseo-make/) — original make template
