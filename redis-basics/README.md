...
Redis Basics is the smallest possible working example of the Robomotion Redis package. It opens a connection, writes a key, reads it back, and closes out — the exact shape that every non-trivial Redis automation eventually grows from.

When you run the flow, it stores the string `"hello redis"` under the key `my_key` and reads the value back into `msg.result`. If the flow finishes with exit status success, your credentials, networking, and package wiring are all correct.
...
The flow uses four Redis nodes from the `Robomotion.Redis` package plus the standard Start/Stop trigger. `Connect` opens a session against your Redis server and returns a `client_id`. Every downstream Redis node takes that `client_id` as input, so only one authentication round-trip happens per flow run. `Disconnect` closes the socket at the end.
