# Pipedrive Lead Traffic Stats

Every 15 minutes, takes the leads Pipedrive has touched most recently, turns each website into a bare domain, estimates how much organic search traffic that domain gets, and writes the numbers back onto the Pipedrive record.
...
1. **Every 15 Minutes** — an Inject trigger on the source scenario's 900-second interval. **Set Parameters** holds
   ...
   enrichment value lands on the Pipedrive side.
3. **List Recent Contacts** — reads the ten most recently touched records via the `GetAll` node against `person` records.
...
6. **Update Contact** — writes back through the `Update` node with a data object keyed by custom field hash.
...
- [Get Traffic Stats for Pipedrive leads with DataForSEO + Make](https://dataforseo.com/templates/get-traffic-stats-for-pipedrive-leads-with-dataforseo-make/) — original make template
