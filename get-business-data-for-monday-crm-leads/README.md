# Monday CRM Lead Business Data

Every 15 minutes, takes the leads monday has touched most recently, turns each website into a bare domain, looks the company up in the Google Business listings database, and writes the name, description, categories, address, rating and price level back onto the monday record.
...
1. **Every 15 Minutes** — an Inject trigger on the source scenario's 900-second interval. **Set Parameters** holds
   ...
   enrichment value lands on the monday side.
...
3. **List Recent Contacts** — reads the ten most recently touched records via a GraphQL `items_page` query against the monday API.
...
6. **Update Contact** — writes back through a GraphQL `change_multiple_column_values` mutation.
...
- **monday credential** — a vault item holding the API token.
...
- [Get Business Data for Monday CRM leads with DataForSEO + Make](https://dataforseo.com/templates/get-business-data-for-monday-crm-leads-with-dataforseo-make/) — original make template
